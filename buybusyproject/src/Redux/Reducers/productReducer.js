...
export const productCancelOrderThunk = createAsyncThunk("product/CancelPerticularOrder",
    async (args, thunkAPI) => {
        // getting user's data 
        const { authReducer, productReducer } = thunkAPI.getState();
        const { userLoggedIn } = authReducer;
        ...
        const currentProduct = productReducer.myorders[args];
        const currentDate = getDate().split("-");
        const productDate = currentProduct.date.split("-");
        if (currentDate[0] === productDate[0] && currentDate[1] === productDate[1] && parseInt(currentDate[2]) - parseInt(productDate[2]) <= 3) {
            const updatedOrders = [...productReducer.myorders.slice(0, args), ...productReducer.myorders.slice(args + 1)];
            ...
            // Return the updated orders to the extraReducer for further operations
...
const productSlice = createSlice({
    ...
    reducers: {
        ...
        setMyOrders: (state, action) => {
            ...
            console.log(state.myorders, "inside setMyOrders");
            return;
        },
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getInitialCartOrdersThunk.fulfilled, (state, action) => {
            ...
            .addCase(productCancelOrderThunk.fulfilled, (state, action) => {
                if (action.payload != null) {
                    state.myorders = action.payload;
                    toast.success("Order cancel successfuly!!");
                } else {
                    toast.error("Your order is placed or on the way");
                }
